/*
 * Storage — reads/writes the shared schedule.json without relying on QML's
 * (disabled-by-default) local-file XHR. Everything goes through the
 * Plasma5Support "executable" engine:  `cat` to read, base64 to write.
 * The file lives at  ~/.local/share/myschedule/schedule.json  and is shared
 * with the standalone planner app.
 */
import QtQuick
...
Item {
    ...
    function _run(cmd, cb) {
        // Append a unique shell comment so two identical commands never collide
        // in the callback map.
        storage._nonce += 1;
